...
pkg_setup() {
	...
	# group + user can be fixed up afterwards for GLEP-81 migration.
	enewgroup prometheus-exporter
	enewuser prometheus-exporter -1 -1 -1
}

src_compile() {

	# TODO: some ebuilds which inherit go-module export some paths here, do we need to?
